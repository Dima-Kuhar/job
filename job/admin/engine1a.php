...
// Updated 09.12.2004: Можно ставить тип SQLTIMESTAMP
//====================================================
...
function show_content()
{
	...
	for ($i=0;$i<count($rows);$i++)
	{
		...
		{
			if($db_fields_assoc[$j]["fld_form"]=="LISTBOX" && ($db_fields_assoc[$j]["fld_preview"]=="PREVIEW" || $db_fields_assoc[$j]["fld_preview"]=="PREVIEW|LINK"))
			{
				$cur_list='list_'.$db_fields_assoc[$j]["fld_field"]."_list";
				//echo "<b>'".$rows[$i][$db_fields_assoc[$j]["fld_field"]]."'</b><br>";
				$cur_list_arr=$GLOBALS[$cur_list];
				//echo "<b>'".$cur_list_arr[0]."'</b><br>";
				$cur_list_val=$cur_list_arr[$rows[$i][$db_fields_assoc[$j]["fld_field"]]];
				
				if($db_fields_assoc[$j]["fld_preview"]=="PREVIEW")
					echo "\t<td><div class=\"fnt2\">".$cur_list_val."</div></td>\n";
				elseif($db_fields_assoc[$j]["fld_preview"]=="PREVIEW|LINK")
					echo "\t<td><div class=\"fnt2\"><a href=\"?admin_action=form_edit&page=$page&id=".$rows[$i][$db_id_field]."\">".$cur_list_val."</a></div></td>\n";
			}
			else
			{
				if($db_fields_assoc[$j]["fld_preview"]=="PREVIEW")
...
function show_form($action,$preset_values='',$errors='')
{
	...
	for ($i=0;$i<$db_fields_num;$i++)
	{
		...
		if($db_fields_assoc[$i]["fld_show".$action_type]=='SHOW '.strtoupper($action_type))
		{
			...
			// то некоторые типы полей имеют установки по умолчанию (DATE)
			else
				$form_value=$form_value=compose_form_value($db_fields_assoc[$i]["fld_type"]);
	...
	for ($i=0;$i<$db_fields_num;$i++)
	{
		...
		if($type_expl[0]=='FILE' && $GLOBALS[$db_fields_assoc[$i]["fld_field"]]!='')
		{
			if($type_expl[1]=='FOTO')
			{
				...
				{
					@$s=do_small_copy($GLOBALS[$db_fields_assoc[$i]["fld_field"]], $DOCUMENT_ROOT.$file_dir.'small/'.$form_post_vars[$db_fields_assoc[$i]["fld_field"]], $smallimg_x, $smallimg_y);
	...
	for ($i=0;$i<$db_fields_num;$i++)
	{
		...
		if($type_expl[0]=='FILE' && isset($form_post_vars[$db_fields_assoc[$i]["fld_field"]]))
		{
			...
			if($type_expl[1]=='FOTO')
			{
				...
				if(@$type_expl[3]=='SMALL')
				{
					@$s=do_small_copy($GLOBALS[$db_fields_assoc[$i]["fld_field"]], $DOCUMENT_ROOT.$file_dir.'small/'.$filename, $smallimg_x, $smallimg_y);
					if(!$s) echo "Ошибка при создании уменьшенной копии файла";
				}
			}
			if($type_expl[1]=='ECSV')
