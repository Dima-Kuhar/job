...
switch($action)
{
	...
	case 'search':
		...
		$form_vars['pay']           = trim(strip_tags(mysql_real_escape_string($HTTP_GET_VARS['pay'])));
		$form_vars['sex']           = trim(strip_tags(mysql_real_escape_string($HTTP_GET_VARS['sex'])));
		$form_vars['age_min']       = trim(strip_tags(mysql_real_escape_string($HTTP_GET_VARS['age_min'])));
		$form_vars['age_max']       = trim(strip_tags(mysql_real_escape_string($HTTP_GET_VARS['age_max'])));
		...
		if($check_form == '')
		{
			...
			if(count($res)>0)
			{
				echo '<h1>ПОИСК РЕЗЮМЕ</h1>'."\n";
				?>
				<table border="0" width="100%" height="25" cellpadding="4" cellspacing="0" background="/images/bg-top02.gif">
				<tr>
					...
					<td valign="middle" background="/images/bg-top02-0.gif" align=right><div class="text3"><b style='padding-right: 50%'>&nbsp;<?echo $links;?></b></div></td>
				</tr>
				</table>
				...
				<?
				echo '<br><p><a style="font-size: 105%;" href="findresume.html">Новый поиск</a>';
			}
			// ничего не найдено
			else
			{
				...
				echo '<p><a href="findresume.html">Попробуйте еще раз</a>';
			}
		}
		else
		{
...
function show_search_form($values='', $errors='')
{
	...
	<form name="search_form" method="get" enctype="multipart/form-data" action="<?=$GLOBALS['SCRIPT_NAME'];?>" class=tform>
		...
		if($values!='' && isset($values['sex']))
		{
			...
			echo "\t\t\t".'<option value="0" '.$sex_0.'>не важно'."\n";
		...
		if($cheked == '' || $cheked == 0)
			echo "\t\t\t".'<option value="0" selected>любой'."\n";
...
function check_vars($form_vars)
{
	...
	if($form_vars['sex']!='m' && $form_vars['sex']!='f' && $form_vars['sex']!='0')
		$error['sex']='Вы не указали свой пол';

	if($form_vars['age_min']!='' && !check_integer($form_vars['age_min'], true) || $form_vars['age_min']>100)
		$error['age_min']='Указан некорректный возраст';

	if($form_vars['age_max']!='' && !check_integer($form_vars['age_max'], true) || $form_vars['age_max']>100)
		$error['age_max']='Указан некорректный возраст';
	elseif($form_vars['age_min']!='' && $form_vars['age_max']!='' && $form_vars['age_max']<$form_vars['age_min'])
...
{
	global $db_users, $db_resume;
	global $db_citys, $db_specialitys;
	global $page, $rowsonpage;

	$fields    = "$db_resume.id, $db_specialitys.speciality, $db_users.sex, $db_users.age, $db_citys.city, $db_resume.pay, $db_resume.date, $db_resume.currency";
	$tables    = "$db_resume, $db_users, $db_citys, $db_specialitys";
	$condition = "$db_resume.ruscable_user_id = $db_users.ruscable_user_id and $db_resume.speciality_id = $db_specialitys.id and $db_users.city_id = $db_citys.id";
	...
	if($form_vars['keywords'] != '')
	{
		...
		for($i=0; $i<count($words); $i++)
		{
			...
			if($i != 0) $condition .= " or ";
			$condition .= "$db_resume.education LIKE '%".$words[$i]."%'";
			$condition .= " or $db_specialitys.speciality LIKE '%".$words[$i]."%'";
			$condition .= " or $db_resume.experience LIKE '%".$words[$i]."%'";
			$condition .= " or $db_resume.skill LIKE '%".$words[$i]."%'";
		}
		$condition .= ")";
	}
	// график
	if($form_vars['schedule_id'] != 1)
		$condition .= " and ($db_resume.schedule_id='".$form_vars['schedule_id']."' or $db_resume.schedule_id='1')";
	// з/п
	if($form_vars['pay'] != ''){
		$pay2=intval($form_vars['pay']/30);
		$condition .= " and ( ($db_resume.currency=1 and $db_resume.pay>='".$form_vars['pay']."') or ($db_resume.currency=0 and $db_resume.pay>='".$pay2."') ) ";
	}
	// пол
	if($form_vars['sex'] != 0)
		$condition .= " and $db_users.sex='".$form_vars['sex']."'";
	// возраст мин
	if($form_vars['age_min'] != '')
		$condition .= " and $db_users.age>='".$form_vars['age_min']."'";
	// возраст макс
	if($form_vars['age_max'] != '')
		$condition .= " and $db_users.age<='".$form_vars['age_max']."'";
	// город
	if($form_vars['city_id'] != 0)
		$condition .= " and $db_users.city_id='".$form_vars['city_id']."'";
	// срок
	if($form_vars['lastdate'] != 0)
		$condition .= " and $db_resume.date>'".date("Y-m-d 00:00:00", (time()-$form_vars['lastdate']*60*60))."'";
...
function show_results($rows)
{
	...
	for ($i=0;$i<count($rows);$i++)
	{
		...
		if($rows[$i]['sex']=='m') $sex='Мужчина';
		elseif($rows[$i]['sex']=='f') $sex='Женщина';
		$rows[$i]['speciality']=ucfirst(trim($rows[$i]['speciality']));
		echo "\t".'<li><a style="font-size: 105%;" href="showresume-'.$rows[$i]['id'].'.html?"><b>'.ucfirst($rows[$i]['speciality']).'</b></a><br>'."\n";
		echo "\t".$sex.', '.$rows[$i]['age'].' лет, проживает в городе '.$rows[$i]['city'].'<br>'."\n";
