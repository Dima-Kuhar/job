...
$link_filename='showallvacancie.html';
...

{
...

	{
		...
		echo "\t".'<li><a style="font-size: 105%;"  href="showvacancie-'.$rows[$i]['id'].'.html"><b>'.ucfirst($speciality).'</b></a><br>'."\n";

		echo "\t".$name.', город '.$city.'<br>'."\n";

		if ($rows[$i]['pay']>0){
			if($rows[$i]['currency']==0){
				$col=$rows[$i]['pay']*30;
			}else{
				$col=$rows[$i]['pay'];
			}
			...
			echo "\t".'<b>Зарплата:</b> по результатам собеседования<br>'."\n";
		}
