...
//====================================================
// если пользователь не залогинен, то отсылаем его на страницу addvacancie.html
...
switch($action)
{
	...
	case 'do_add':
		// принимаем данные
		// шаг 1: Данные об интересующем соискателе
		...
		if($check_form=='')
		{
			...
			else $error_log[] = 'Не удалось добавить вакансию в таблицу';
			// выводим пользователю отчет
			if(count($error_log) == 0)
			{
				$action_inform='Ваша вакансия добавлена в базу данных Job.RusCable.Ru';
				$msg=create_letter2($user_dbid, $form_vars, date("Y/m/d H:i"), 'newvacancie');
				letter_2_manager($msg, 'Новая вакансия');
				include('0-header.inc');
				echo '<h1 align="center">Добавление вакансии</h1>'."\n";
				show_action_inform();
				echo '<p>Постоянный адрес Вашей вакансии:<br>'."\n";
			...
			{
				$action_inform='Извините, в данный момент добавление Вашей вакансии невозможно. Сервис будет доступен в течении часа.';
				$to_admin = implode("\n", $error_log);
...
function show_form($values='', $errors='')
{
	...
	<input type="submit" name="submit" value="Добавить вакансию" class="ff">
